...
impl<'a> CodeGenerator<'a> {
    ...
    fn gen_service(&mut self, s: &Service) {
        ...
        writeln!(self.output, "/// Server for the {} service.", service_name).unwrap();
        writeln!(
            self.output,
            "pub struct {}Server<Tr: __rt::rpc::Transport> {{",
            service_name
        )
        .unwrap();
        writeln!(self.output, "    base: __rt::rpc::ServerBase<Tr>,").unwrap();
        writeln!(self.output, "}}\n").unwrap();

        // Generate server impl
        writeln!(
            self.output,
            "impl<Tr: __rt::rpc::Transport> {}Server<Tr> {{",
            service_name
        )
        .unwrap();
        ...
        writeln!(
            self.output,
            "    /// Creates a new server for the {} service.",
            service_name
        )
        .unwrap();
        writeln!(
            self.output,
            "    pub fn new<T: {} + 'static>(transport: Tr, impl_: T) -> Self {{",
            service_name
        )
        .unwrap();
        writeln!(
            self.output,
            "        let mut base = __rt::rpc::ServerBase::new(transport);"
        )
        .unwrap();
        writeln!(self.output, "        let impl_ = ::std::rc::Rc::new(impl_);").unwrap();
        ...
        for method in &s.methods {
            ...
            match &method.response.node {
                ServiceResponse::Unary(resp_ty) => {
                    writeln!(
                        self.output,
                        "            base.register_unary({}, move |payload| {{",
                        index
                    )
                    .unwrap();
                    writeln!(self.output, "                let impl_ = impl_.clone();").unwrap();
                    writeln!(self.output, "                async move {{").unwrap();

                    // Decode the request
                    writeln!(self.output, "                    let mut buf = payload.as_slice();").unwrap();
                    self.gen_decode_for_service(&method.request.node, 20);

                    // Call the implementation
                    writeln!(
                        self.output,
                        "                    let resp = impl_.{}(req).await?;",
                        method_name
                    )
                    .unwrap();
                    ...
                    .unwrap();
                    self.gen_encode_for_service("resp", resp_ty, 20, "resp_buf");
                    writeln!(self.output, "                    ::core::result::Result::Ok(resp_buf)").unwrap();
                    writeln!(self.output, "                }}").unwrap();
                    writeln!(self.output, "            }});").unwrap();
                }
                ServiceResponse::Stream(resp_ty) => {
                    writeln!(
                        self.output,
                        "            base.register_stream({}, move |payload, stream| {{",
                        index
                    )
                    .unwrap();
                    writeln!(self.output, "                let impl_ = impl_.clone();").unwrap();
                    ...
                    // Decode the request - errors are reported via stream sender
                    writeln!(self.output, "                    let mut buf = payload.as_slice();").unwrap();
                    writeln!(
                        self.output,
                    ...
                    writeln!(
                        self.output,
                        "                            stream.error(__rt::rpc::ERR_CODE_DECODE_ERROR, &format!(\"decode error: {{}}\", e)).await;"
                    )
                    .unwrap();
                    ...
                    // Create typed stream sender wrapping the raw sender
                    writeln!(
                        self.output,
                        "                    let typed_stream = __rt::rpc::StreamSender::<{}>::new(stream);",
                        self.rust_type(resp_ty)
                    )
                    .unwrap();
                    writeln!(
                        self.output,
                        "                    impl_.{}(req, typed_stream).await;",
                        method_name
                    )
                    .unwrap();
                    writeln!(self.output, "                }}").unwrap();
        ...
        writeln!(self.output, "        Self {{ base }}").unwrap();
        writeln!(self.output, "    }}\n").unwrap();
        ...
        writeln!(
            self.output,
            "    pub async fn serve(self) -> ::core::result::Result<(), __rt::rpc::RpcError> {{"
        )
        .unwrap();
        writeln!(self.output, "        self.base.serve().await").unwrap();
        writeln!(self.output, "    }}").unwrap();
        writeln!(self.output, "}}\n").unwrap();
        ...
        writeln!(
            self.output,
            "pub struct {}Client<Tr: __rt::rpc::Transport> {{",
            service_name
        )
        .unwrap();
        writeln!(self.output, "    base: ::std::rc::Rc<__rt::rpc::ClientBase<Tr>>,").unwrap();
        writeln!(self.output, "}}\n").unwrap();

        // Generate client impl
        writeln!(
            self.output,
            "impl<Tr: __rt::rpc::Transport> {}Client<Tr> {{",
            service_name
        )
        .unwrap();
        ...
        writeln!(self.output, "        Self {{").unwrap();
        writeln!(
            self.output,
            "            base: ::std::rc::Rc::new(__rt::rpc::ClientBase::new(transport)),"
        )
        .unwrap();
        writeln!(self.output, "        }}").unwrap();
        writeln!(self.output, "    }}\n").unwrap();
        ...
        writeln!(
            self.output,
            "    /// Runs the client's send and receive loops. Must be spawned before making RPC calls."
        )
        .unwrap();
        writeln!(
            self.output,
            "    pub async fn run(&self) -> ::core::result::Result<(), __rt::rpc::RpcError> {{"
        )
        .unwrap();
        writeln!(self.output, "        self.base.run().await").unwrap();
        writeln!(self.output, "    }}\n").unwrap();
        ...
        for method in &s.methods {
            ...
            match &method.response.node {
                ServiceResponse::Unary(resp_ty) => {
                    ...
                    writeln!(
                        self.output,
                        "        let resp_buf = self.base.call_unary({}, req_buf).await?;",
                        index
                    )
                    .unwrap();
                ...
                ServiceResponse::Stream(resp_ty) => {
                    ...
                    writeln!(
                        self.output,
                        "        let receiver = self.base.call_stream({}, req_buf).await?;",
                        index
                    )
                    .unwrap();
